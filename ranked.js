import {rankedConfig} from './ranked-config.js?v=20260916-auth-2';
...
export async function authClient(){
 ...
 if(!clientPromise)clientPromise=import('https://esm.sh/@supabase/supabase-js@2.116.0').then(({createClient})=>createClient(rankedConfig.url,rankedConfig.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}})).catch(error=>{clientPromise=null;throw error;});
...
export async function observeAuth(onChange){
 const c=await authClient();
 const {data:{subscription}}=c.auth.onAuthStateChange((_event,session)=>onChange(session));
 const {data,error}=await c.auth.getSession();
 if(error){subscription.unsubscribe();throw error;}
